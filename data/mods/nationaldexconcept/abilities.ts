export const Abilities: {[k: string]: ModdedAbilityData} = {
	...
	aurabreak: {
		...
		rating: 1,
		num: 188,
		shortDesc: "While this Pokemon is active, Terrain and Dark/Fairy Aura have a 0.75x power modifier.",
	},
	baddreams: {
		onResidualOrder: 28,
	...
	ballfetch: {
		...
		rating: 0,
		num: 237,
		shortDesc: "This Pokemon's attacks have the Fling effect that their item has.",
	},
	battery: {
	...
	bigpecks: {
		...
		rating: 0.5,
		num: 145,
		shortDesc: "This Pokemon is immune to Stealth Rock.",
	},
	blaze: {
	...
	curiousmedicine: {
		...
		rating: 0,
		num: 261,
		shortDesc: "On switch-in, opposing Pokemon have their stat stages reset to 0.",
	},
	cursedbody: {
		onDamagingHit(damage, target, source, move) {
	...
	defeatist: {
		...
		rating: -1,
		num: 129,
		shortDesc: "When maximum HP ≤ 1/2, this Pokemon's Attack is lowered by 1."
	},
	defiant: {
		onAfterEachBoost(boost, target, source, effect) {
			if (!source || target.isAlly(source)) {
	...
	flareboost: {
		...
		rating: 2,
		num: 138,
		shortDesc: "While this Pokemon is burned, its Special Attack is 1.5x; ignores burn damage.",
	},
	flashfire: {
		onTryHit(target, source, move) {
	...
	healer: {
		onSetStatus(status, target, source, effect) {
			if ((effect as Move)?.status) {
				this.add('-immune', target, '[from] ability: Healer');
			}
			return false;
		},
		...
		rating: 0,
		num: 131,
		shortDesc: "Immune to status conditions and Attract/Disable/Encore/Heal Block/Taunt/Torment."
	},
	heatproof: {
		...
		rating: 2,
		num: 85,
		shortDesc: "Fire damage against this Pokemon is halved; burn immunity.",
	},
	heavymetal: {
	...
	honeygather: {
		onDamagingHit(damage, target, source, move) {
			if (this.checkMoveMakesContact(move, source, target, true)) {
				this.add('-ability', target, 'Honey Gather');
				this.boost({spe: -1}, source, target, null, true);
			}
		},
		flags: {},
	...
	keeneye: {
		...
		onSourceModifyAccuracyPriority: 5,
		onSourceModifyAccuracy(accuracy, target, source, move) {
				return this.chainModify([4915, 4096]);
		},
		flags: {breakable: 1},
		...
		rating: 0.5,
		num: 51,
		shortDesc: "This Pokemon's accuracy is multiplied by 1.2x; cannot be lowered."
	},
	klutz: {
	...
	magician: {
		...
		rating: 1,
		num: 170,
		shortDesc: "This Pokemon's attacks have a 20% chance to remove the opponents item."
	},
	magmaarmor: {
		onUpdate(pokemon) {
	...
	watercompaction: {
		...
		onAnyRedirectTarget(target, source, source2, move) {
			...
			if (this.validTarget(this.effectState.target, source, redirectTarget)) {
				if (move.smartTarget) move.smartTarget = false;
				if (this.effectState.target !== target) {
					this.add('-activate', this.effectState.target, 'ability: Water Compaction');
				}
				return this.effectState.target;
			}
		},
	...
	zerotohero: {
		...
		rating: 5,
		num: 278,
		shortDesc: "Changes the forme of Palafin once switched out."
	},
	windsummoner: {
		onStart(source) {
			this.field.setWeather('windystream');
		},
		flags: {},
		name: "Wind Summoner",
		rating: 4,
	...
	ancientawakening: {
		onSwitchOut(pokemon) {
			if (pokemon.baseSpecies.baseSpecies !== 'Unown') return;
			if (pokemon.species.forme !== 'Complete') {
				pokemon.formeChange('Unown-Complete', this.effect, true);
			}
		},
		onSwitchIn() {
			this.effectState.switchingIn = true;
		...
		onStart(pokemon) {
			...
			this.effectState.switchingIn = false;
			if (pokemon.baseSpecies.baseSpecies !== 'Unown') return;
			if (!this.effectState.heroMessageDisplayed && pokemon.species.forme === 'Complete') {
				this.add('-activate', pokemon, 'ability: Ancient Awakening');
				this.effectState.completeMessageDisplayed = true;
			}
		},
		flags: {failroleplay: 1, noreceiver: 1, noentrain: 1, notrace: 1, failskillswap: 1, cantsuppress: 1, notransform: 1},
		...
		rating: 5,
		num: 278,
		shortDesc: "If Unown, changes to Complete form once switched out."
	},
	zerotoheroledian: {
		onSwitchOut(pokemon) {
			if (pokemon.baseSpecies.baseSpecies !== 'Ledian') return;
			if (pokemon.species.forme !== 'Hero') {
				pokemon.formeChange('Ledian-Hero', this.effect, true);
			}
		},
		onSwitchIn() {
			this.effectState.switchingIn = true;
		...
		onStart(pokemon) {
			...
			if (pokemon.baseSpecies.baseSpecies !== 'Ledian') return;
			if (!this.effectState.heroMessageDisplayed && pokemon.species.forme === 'Hero') {
				this.add('-activate', pokemon, 'ability: Zero to Hero (Ledian)');
				this.effectState.heroMessageDisplayed = true;
			}
		},
		flags: {failroleplay: 1, noreceiver: 1, noentrain: 1, notrace: 1, failskillswap: 1, cantsuppress: 1, notransform: 1},
		name: "Zero to Hero (Ledian)",
		rating: 5,
		num: 278,
