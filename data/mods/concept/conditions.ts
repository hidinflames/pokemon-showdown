export const Conditions: {[k: string]: ModdedConditionData} = {
	...
	windystream: {
		...
		onWeatherModifyDamage(damage, attacker, defender, move) {
			if (defender.hasItem('utilityumbrella')) return;
			if (move.type === 'Flying') {
				this.debug('Windy Stream Flying boost');
				return this.chainModify(1.5);
			}
			if (move.type === 'Electric') {
				this.debug('Windy Stream Electric suppress');
				return this.chainModify(0.5);
			}
		},
		onFieldStart(field, source, effect) {
			if (effect?.effectType === 'Ability') {
				if (this.gen <= 5) this.effectState.duration = 0;
				this.add('-weather', 'WindyStream', '[from] ability: ' + effect.name, '[of] ' + source);
			} else {
				this.add('-weather', 'WindyStream');
			}
		},
		onFieldResidualOrder: 1,
		onFieldResidual() {
			this.add('-weather', 'WindyStream', '[upkeep]');
			this.eachEvent('Weather');
		},
		onFieldEnd() {
			this.add('-weather', 'none');
