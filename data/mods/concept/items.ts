export const Items: {[k: string]: ModdedItemData} = {
	abilityshield: {
		...
		auspiciousarmor: {
			...
			onBasePowerPriority: 15,
			onBasePower(basePower, user, target, move) {
			if ((move.type === 'Fire' || move.type === 'Psychic')) {
				return this.chainModify([4915, 4096]);
			}
			},
			num: 2344,
			gen: 9,
			shortDesc: "This Pokemon's Fire and Psychic moves have 1.2x power.",
		},
		babiriberry: {
			name: "Babiri Berry",
			spritenum: 17,
		...
		dubiousdisc: {
			...
			onModifySpAPriority: 1,
			onModifySpA(spa, pokemon) {
			if (pokemon.baseSpecies.baseSpecies === 'Porygon-Z') {
				return this.chainModify(1.5);
			}
			},
			onResidualOrder: 5,
			onResidualSubOrder: 4,
			onResidual(pokemon) {
				if ((pokemon.baseSpecies.num === 474)) {
					this.heal(pokemon.baseMaxhp / 16);
				} else {
					this.damage(pokemon.baseMaxhp / 8);
				}
			...
			num: 324,
			gen: 4,
			shortDesc: "If holder is Porygon-Z, heals 1/16 HP per turn and Special Attack is 1.5x.",
		},
		durinberry: {
			name: "Durin Berry",
			spritenum: 114,
		...
		electirizer: {
			...
			onResidualSubOrder: 4,
			onResidual(pokemon) {
				if ((pokemon.baseSpecies.num === 466)) {
					this.heal(pokemon.baseMaxhp / 16);
				} else {
					this.damage(pokemon.baseMaxhp / 8);
				}
			...
			num: 322,
			gen: 4,
			shortDesc: "If holder is Electivire, heals 1/16 HP per turn and Attack is 1.5x.",
		},
		electricgem: {
			name: "Electric Gem",
			spritenum: 120,
		...
		magmarizer: {
			...
			onResidualSubOrder: 4,
			onResidual(pokemon) {
				if ((pokemon.baseSpecies.num === 467)) {
					this.heal(pokemon.baseMaxhp / 16);
				} else {
					this.damage(pokemon.baseMaxhp / 8);
				}
			...
			num: 323,
			gen: 4,
			shortDesc: "If holder is Magmortar, heals 1/16 per turn and Special Defense is 1.5x.",
		},
		magnet: {
			name: "Magnet",
			spritenum: 273,
		...
		maliciousarmor: {
			...
			num: 1861,
			gen: 9,
			shortDesc: "This Pokemon's Fire and Ghost moves have 1.2x power.",
		},
		manectite: {
			name: "Manectite",
			spritenum: 596,
		...
		protector: {
			...
			onResidualSubOrder: 4,
			onResidual(pokemon) {
				if ((pokemon.baseSpecies.num === 464)) {
					this.heal(pokemon.baseMaxhp / 16);
				} else {
					this.damage(pokemon.baseMaxhp / 8);
				}
			...
			num: 321,
			gen: 4,
			shortDesc: "If holder is Rhyperior, heals 1/16 HP per turn and Defense is 1.5x.",
		},
		psychicgem: {
			name: "Psychic Gem",
			spritenum: 369,
		...
		reapercloth: {
			...
			onResidualSubOrder: 4,
			onResidual(pokemon) {
				if ((pokemon.baseSpecies.num === 477)) {
					this.heal(pokemon.baseMaxhp / 16);
				} else {
					this.damage(pokemon.baseMaxhp / 8);
				}
			...
			num: 325,
			gen: 4,
			shortDesc: "If holder is Dusknoir, heals 1/16 per turn and its Speed stat is modified by 1.5x.",
		},
		redcard: {
			name: "Red Card",
			spritenum: 387,
		...
		sweetapple: {
			...
			num: 1116,
			gen: 8,
			shortDesc: "This Pokemon's moves have a 10% chance to confuse.",
		},
		syrupyapple: {
			name: "Syrupy Apple",
			spritenum: 755,
			...
			onModifyMove(move) {
				if (move.category !== "Status") {
					if (!move.secondaries) move.secondaries = [];
					for (const secondary of move.secondaries) {
						if (secondary.volatileStatus === 'par') return;
					}
					move.secondaries.push({
						chance: 10,
						volatileStatus: 'par',
			...
			num: 2402,
			gen: 9,
			shortDesc: "This Pokemon's moves have a 10% chance to paralyze.",
		},
		tamatoberry: {
			name: "Tamato Berry",
			spritenum: 486,
		...
		tartapple: {
			...
			onModifyMove(move) {
				if (move.category !== "Status") {
					if (!move.secondaries) move.secondaries = [];
					for (const secondary of move.secondaries) {
						if (secondary.volatileStatus === 'psn') return;
					}
					move.secondaries.push({
						chance: 10,
						volatileStatus: 'psn',
			...
			num: 1117,
			gen: 8,
			shortDesc: "This Pokemon's moves have a 10% chance to poison.",
		},
		terrainextender: {
