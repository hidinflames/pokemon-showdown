...
export const Rulesets: {[k: string]: FormatData} = {
	...
	standardnatdexoms: {
		effectType: 'ValidatorRule',
		name: 'Standard NatDex OMs',
		desc: "The standard ruleset for all NatDex OMs",
		ruleset: [
			'Obtainable', '+Unobtainable', '+Past', 'Sketch Post-Gen 7 Moves', 'Team Preview', 'Species Clause', 'Nickname Clause', 'OHKO Clause', 'Evasion Moves Clause', 'Endless Battle Clause', 'HP Percentage Mod', 'Cancel Mod', 'Overflow Stat Mod',
			// 'Min Source Gen = 9', - crashes for some reason
		],
		onValidateSet(set) {
			const species = this.dex.species.get(set.species);
	...
	standardnatdexnfe: {
		...
		desc: "The standard ruleset for all National Dex NFE tiers",
		ruleset: [
			'Obtainable', '+Unobtainable', '+Past', 'Sketch Post-Gen 7 Moves', 'Team Preview', 'Nickname Clause', 'HP Percentage Mod', 'Cancel Mod', 'Endless Battle Clause',
		],
		onValidateSet(set) {
	...
	standardnatdexlc: {
		effectType: 'ValidatorRule',
		name: 'Standard NatDex LC',
		desc: "The standard ruleset for all National Dex LC tiers",
		ruleset: [
			'Max Level = 5', 'Obtainable', '+Unobtainable', '+Past', 'Sketch Post-Gen 7 Moves', 'Team Preview', 'Nickname Clause', 'HP Percentage Mod', 'Cancel Mod', 'Endless Battle Clause',
		],
		onValidateSet(set) {
			const species = this.dex.species.get(set.species);
			if (species.natDexTier === 'Illegal') {
	...
	convergencelegality: {
		...
		onValidateSet(set, format) {
			...
			const matchingSpecies = this.dex.species.all()
				.filter(species => (
					(!species.isNonstandard || this.ruleTable.has(`+pokemontag:${this.toID(species.isNonstandard)}`)) && !this.ruleTable.has('standardnatdex') &&
		...
		checkCanLearn(move, species, setSources, set) {
			const matchingSpecies = this.dex.species.all()
				.filter(s => (
					(!s.isNonstandard || this.ruleTable.has(`+pokemontag:${this.toID(s.isNonstandard)}`)) && !this.ruleTable.has('standardnatdex') &&
